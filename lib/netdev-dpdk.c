...
#ifdef PPS_RATE_LIMIT
	netdev->sla_pps = smap_get_ullong(args, "limit-pps", DEFAULT_LIMIT_PPS);
	netdev->pps_bucket = netdev->sla_pps;
	netdev->last_tsc = rte_get_tsc_cycles();
#else 
#ifdef RTFQ_RATE_LIMIT
	netdev->sla_rt = smap_get_ullong(args, "limit-cycles", DEFAULT_LIMIT_RT);
        netdev->rt_bucket = DEFAULT_BURST_CYCLE;
	netdev->last_tsc = rte_get_tsc_cycles();
#endif
#endif
                           ...
                           struct dp_packet_batch *batch)
{
	...
	interval = current_tsc - rxq->netdev->last_tsc;
	rxq->netdev->last_tsc = current_tsc;

	double_token = (double) rxq->netdev->sla_pps * interval / (rte_get_tsc_hz());
	
	rxq->netdev->last_token += double_token;
	int_token = rxq->netdev->last_token;
	rxq->netdev->last_token -= int_token;
	rxq->netdev->current_token += int_token;

	if (rxq->netdev->current_token > rxq->netdev->pps_bucket) {
		rxq->netdev->current_token = rxq->netdev->pps_bucket;
	}

	if( rxq->netdev->current_token < nb_rx ) {
		mbuf_pkt = (struct rte_mbuf **) batch->packets;
		for (unsigned int i = rxq->netdev->current_token; i < nb_rx; i++) {
			rte_pktmbuf_free(mbuf_pkt[i]);
		}
		dropped = nb_rx - rxq->netdev->current_token;
		nb_rx = rxq->netdev->current_token;
		rxq->netdev->current_token = 0;
	} else {
		rxq->netdev->current_token -= nb_rx;
	}
