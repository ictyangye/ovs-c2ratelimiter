...
enum rxq_cycles_counter_type {
	...
	RXQ_CYCLES_PROC_RECV,
	RXQ_CYCLES_PROC_LOOKUP,
	RXQ_CYCLES_PROC_SEND,
    RXQ_N_CYCLES
};
                           ...
                           odp_port_t port_no)
{
    ...
    if (!error) {
	    ...
	    cycles1 = cycle_timer_stop(&pmd->perf_stats, &timer);
		dp_netdev_rxq_add_cycles(rxq, RXQ_CYCLES_PROC_RECV, cycles1);
	    cycle_timer_start(&pmd->perf_stats, &timer);
        *recirc_depth_get() = 0;
        pmd_thread_ctx_time_update(pmd);
        ...
        cycles2 = cycle_timer_stop(&pmd->perf_stats, &timer);
		dp_netdev_rxq_add_cycles(rxq, RXQ_CYCLES_PROC_LOOKUP, cycles2);
        dp_netdev_rxq_add_cycles(rxq, RXQ_CYCLES_PROC_CURR, cycles1+cycles2);
	...
	currentdev = netdev_rxq_get_netdev_externalcall(rxq->rx);
	currentdev->pps += output_cnt;
...
pmd_thread_main(void *f_)
{
	...
	uint64_t start_tsc,end_tsc,interval;
	double double_rt;
	struct netdev *currentdev;
	int64_t int_token;
    poll_list = NULL;
...
reload:
	...
	//pmd->last_scheduled_tsc = rte_get_tsc_cycles();
    ...
    for (i = 0; i < poll_cnt; i++) {
	   ...
	   dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_RECV, 0);
	   dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_LOOKUP, 0);
	   dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_SEND, 0);
	   currentdev = netdev_rxq_get_netdev_externalcall(poll_list[i].rxq->rx);
	   currentdev->pps = 0;
	   currentdev->all_tsc = 0;
    }
    ...
    for (;;) {
        ...
        for (i = 0; i < poll_cnt; i++) {
...
#ifdef RTFQ_RATE_LIMIT

			if (currentdev->netdev_class->type == "dpdkvhostuserclient"){

				interval = start_tsc - currentdev->last_tsc;
				currentdev->last_tsc = start_tsc;
		
				//int_token = (int64_t) currentdev->sla_rt * interval / (rte_get_tsc_hz());
				double_rt = (double) currentdev->sla_rt * interval / (rte_get_tsc_hz());
				currentdev->last_rt += double_rt;
				int_token = currentdev->last_rt;
				currentdev->last_rt -= int_token;
				currentdev->current_rt += int_token;
		
				if (currentdev->current_rt > currentdev->rt_bucket) {
					currentdev->current_rt = currentdev->rt_bucket;
				}
				else if (currentdev->current_rt < 0){
					continue;
				}
			}
			...
			interval = rte_get_tsc_cycles() - start_tsc;
			currentdev->all_tsc += interval;
			currentdev->loop ++;
#ifdef RTFQ_RATE_LIMIT
			if (currentdev->netdev_class->type == "dpdkvhostuserclient"){
				currentdev->current_rt -= interval;
			}
#endif
        }
                  ...
                  bool md_is_valid, odp_port_t port_no)
{
     ...
     * recirculation. When a packet matching flow â€˜jâ€?happens to be
     * recirculated, the nested call to dp_netdev_input__() could potentially
     * classify the packet as matching another flow - say 'k'. It could happen
     * that in the previous call to dp_netdev_input__() that same flow 'k' had
     * already its own batches[k] still waiting to be served.  So if its
     * â€˜batchâ€?member is not reset, the recirculated packet would be wrongly
                           ...
                           struct polled_queue *poll_list, int poll_cnt)
{
    ...
    if (pmd->ctx.now > pmd->rxq_next_cycle_store) {
		current_tsc = rte_get_tsc_cycles();
		scheduled_time = current_tsc - pmd->last_scheduled_tsc;
		pmd->last_scheduled_tsc = rte_get_tsc_cycles();
        uint64_t curr_tsc;
		VLOG_INFO("Port numbers are: %"PRIu32" ", poll_cnt);
        ...
        for (unsigned i = 0; i < poll_cnt; i++) {
            ...
            uint64_t rxq_cyc_recv = dp_netdev_rxq_get_cycles(poll_list[i].rxq,
                                                        RXQ_CYCLES_PROC_RECV);
			uint64_t rxq_cyc_lookup = dp_netdev_rxq_get_cycles(poll_list[i].rxq,
                                                        RXQ_CYCLES_PROC_LOOKUP);
		    uint64_t rxq_cyc_send = dp_netdev_rxq_get_cycles(poll_list[i].rxq,
                                                        RXQ_CYCLES_PROC_SEND);

			VLOG_INFO("Cycles counting: port \'%s\' rx queue %d \'s cycles layout: %"PRIu64",  %"PRIu64",  %"PRIu64" , but all used tsc is  %"PRIu64" ",
                  currentdev->name,netdev_rxq_get_queue_id(poll_list[i].rxq->rx) ,rxq_cyc_recv,rxq_cyc_lookup,rxq_cyc_send, currentdev->all_tsc);
			VLOG_INFO("Rate measured: port \'%s\' \'s PPS: %"PRIu64" , consume loops: %"PRIu64" ",
                  currentdev->name, currentdev->pps*(rte_get_tsc_hz())/scheduled_time, currentdev->loop);
            ...
            dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_CURR,
                                     0);
			dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_RECV,
                                     0);
			dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_LOOKUP,
                                     0);
			dp_netdev_rxq_set_cycles(poll_list[i].rxq, RXQ_CYCLES_PROC_SEND,
                                     0);
			currentdev->pps=0;
			currentdev->all_tsc=0;
			currentdev->loop=0;
